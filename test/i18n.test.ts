...
const KEYS: Key[] = [
  "help", "useInGroupNudge", "needMember", "limitReached",
...
test("t() falls back to English for an unknown language", () => {
  ...
  assert.equal(t("zz", "limitReached", { free: 3, stars: 150 }), t("en", "limitReached", { free: 3, stars: 150 }));
});
...
test("t() substitutes {name} placeholders and leaves no token behind", () => {
  const s = t("en", "limitReached", { free: 3, stars: 150 });
  assert.ok(s.includes("3") && s.includes("150"));
...
test("every language has every btn_* key, non-empty", () => {
  for (const lang of LANGS) {
...
test("commands and the Pro/Stars brand words survive translation", () => {
  for (const lang of LANGS) {
    assert.ok(t(lang, "help", { free: 3, stars: 150 }).includes("/event"));
    assert.ok(t(lang, "help", { free: 3, stars: 150 }).includes("/pro"));
